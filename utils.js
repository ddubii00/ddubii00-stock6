...
async function fallbackQuoteFromSeries(symbol) {
  ...
  return { symbol, price: last, changePercent: ((last - prev) / prev) * 100, asOf, raw: 'fallback-from-series' };
}
...
async function fetchChartSeries(key, interval = '1d') {
  ...
  if (key === 'US2Y') {
    ...
    // supplement Treasury data (omitted for brevity)
    if (!rows.length) return null;
    return rows.slice(-120);
  }
  // USDKRW
  ...
  // VIX, SOX, WTI, DXY, GOLD – similar pattern
  const yahooMap = {
    VIX: '%5EVIX',
    SOX: '%5ESO',
    WTI: 'CL%3DF',
...
module.exports = { fetchStooq, fetchChartSeries, fallbackQuoteFromSeries };
